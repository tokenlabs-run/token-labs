...
DECODE_METRICS_URL="${DECODE_METRICS_URL:-http://165.245.141.196:8020/metrics}"
RESULT_DIR="${RESULT_DIR:-/root/hetero-matrix}"
...
run_point() {
  ...
  curl -fsS "${DECODE_METRICS_URL}" >"${RESULT_DIR}/${point}.before.prom"
  ...
  curl -fsS "${DECODE_METRICS_URL}" >"${RESULT_DIR}/${point}.after.prom"
}
