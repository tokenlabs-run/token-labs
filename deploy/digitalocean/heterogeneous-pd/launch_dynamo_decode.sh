...
: "${PUBLIC_IP:?set PUBLIC_IP to the MI300X public address}"

MODEL="${MODEL:-Qwen/Qwen3-30B-A3B-Instruct-2507-FP8}"
IMAGE="${IMAGE:-token-labs/dynamo-vllm-rocm:1.4.2-vllm0.26-mc0.3.13-bootstrap}"
...
docker rm -f dynamo-decode >/dev/null 2>&1 || true
docker run -d \
  --name dynamo-decode \
  --group-add video \
  --cap-add SYS_PTRACE \
  ...
  -e LD_LIBRARY_PATH=/usr/local/lib/python3.12/dist-packages/mooncake:/usr/local/lib/python3.12/dist-packages/mooncake_transfer_engine_rocm7.libs:/opt/rocm/lib \
